...
import {uuid} from 'pubConf/common.jsx';
...
export default class Pagination extends React.Component {
    ...
    render() {
        ...
        let aFirstPage = this.state.nCurrentPage == 1?[<span className={style.common} key={uuid()} style={{backgroundColor:"rgba(0,0,0,.87)",border:'1px solid rgba(0,0,0,.87)',color:'#ffffff'}} onClick = {thiz.fOnSelectPage.bind(thiz,1)}>1</span>]:[<span key={uuid()} className={style.common} onClick = {thiz.fOnSelectPage.bind(thiz,1)}>1</span>];
        ...
        if (this.state.nCurrentPage-nPrevSpace>2){
            ...
            aStartbypass.push(<span key={uuid()} className={style.common} onClick = {thiz.fByPassPage.bind(thiz,"prev",thiz.state.nCurrentPage-nPrevSpace)}>...</span>)
        }

        if (this.state.nCurrentPage+nEndSpace<this.state.nMaxPage){

            aEndbypass.push(<span key={uuid()} className={style.common} style={{border:'none'}} onClick = {thiz.fByPassPage.bind(thiz,"last",thiz.state.nCurrentPage+nEndSpace)}>...</span>)
        }
        ...
        for (let i = nStart;i<=nEnd;i++){
            ...
            if (i == this.state.nCurrentPage){
                ...
                aMdidllePage.push(<span key={uuid()} className={style.common} style={{backgroundColor:"rgba(0,0,0,.87)",border:'1px solid rgba(0,0,0,.87)',color:'#ffffff'}}>{i}</span>)
            }else{

                aMdidllePage.push(<span key={uuid()} className={style.common} onClick = {thiz.fOnSelectPage.bind(thiz,i)}>{i}</span>)
            }
        }
