...
<html>
	<head>
        ...
        <link rel="stylesheet" type="text/css" href="<?php echo $this->ffwbms_vars["wset_ary"]['tpl_path'] ?>css/jiage.css"/> 
	...
	<div class="row" style="margin-right: 0px;">
		...
		<!--价格-->
			<div class="a"></div>
			<div class="jiage">
				<?php $this->ffwbms_get_value(' ffa_sort_db ',' vte1 ', 'id = 14'); ?><!--获得指定分类号的数据-->
        		<?php $this->ffwbms_get_value(' ffa_art_value ',' vte1_art ', 'id = ' .$this->ffwbms_vars["vte1"][sart]); ?><!--获得指定文章号的文章数据    $vte1[sart]：返回分类所属文章号-->
			    <div class="jiage-title">
					<p style="font-family: '微软雅黑';font-size: 22px;color: #000000;">
						<?php echo $this->ffwbms_vars["vte1"][sname] ?>
					</p>
					<p style="font-family: '微软雅黑';font-size: 14px;color: #8C8C8C;margin-top: 7px;">
						<?php echo $this->ffwbms_vars["vte1"][abs] ?>
					</p>
				</div>
				<div class="jiage-title-content">
			        <?php echo $this->ffwbms_vars["vte1"][atxt] ?>
				</div>
			</div>
			
			<ul class="table clearfix">
				<?php $this->ffwbms_get_value(' ffa_art_pglist ',' vtemp ', 'pid = 14' .'|pg = ' .$this->ffwbms_vars["get_ary"]["pg"] .'|ps = 4|ob = 3'); ?><!--获得文章分页列表-->           
    		...
    		<?php $this->ffwbms_get_value(' ffa_art_value ',' vte1_art ', 'id = ' .$this->ffwbms_vars["vte1"][sart]); ?><!--获得指定文章号的文章数据    $vte1[sart]：返回分类所属文章号-->
			<div class="jiage-img">
				<img src="<?php echo $this->ffwbms_vars["wset_ary"]['upload_path'] ?><?php echo $this->ffwbms_vars["vte1_art"][timgary][0][furl] ?>" alt="">
			</div>
			
			<div style="margin-left:2%;">
				<p style="font-family:'微软雅黑';font-size:13px; text-align:left; padding:2% 0;">
				...
				<div >
					<ul class="clearfix">
						<?php $this->ffwbms_get_value(' ffa_art_pglist ',' vtemp ', 'pid = 14' .'|pg = ' .$this->ffwbms_vars["get_ary"]["pg"] .'|ps = 4|ob = 2'); ?><!--获得文章分页列表-->           
				        <?php foreach( $this->ffwbms_vars["vtemp"][0] as $this->ffwbms_vars["ary"]){ ?>
				        <?php $this->ffwbms_get_value(' ffa_art_value ',' arry ', 'id = ' .$this->ffwbms_vars["ary"][id]); ?>
				        <?php $this->ffwbms_vars["time"] = substr($this->ffwbms_vars["ary"][scdt] , 0 , 10 ) ?>		        
				        <li class="col-xs-12 col-sm-6 col-md-3 col-lg-3">														
							<div class="bank">
								<p style="padding: 1% 0;color: rgb(0, 176, 240);"><?php echo $this->ffwbms_vars["arry"][atit] ?></p>
					...
					<hr style="margin-top: 30px; margin-bottom: 20px; border: 1px dotted #d3d3d3; margin-right: 40px; "/>
				</div>		
				<div>
					<ul class="clearfix">
						<li class="col-xs-12 col-sm-6 col-md-5 col-lg-4">
							<div class="way">
				                <p>②&nbsp; 支付宝转账-扫码付款</p>
				            </div>
				            <div class="account">
				                <p>
				            	    支付宝账号：touch@uelike.com<br />
				                	支付宝名称：北京优艺客文化传播有限公司
				                </p>
				                <img src="<?php echo $this->ffwbms_vars["wset_ary"]['tpl_path'] ?>images/jg8.jpg" >
				            </div>
						</li>
						<li class="col-xs-12 col-sm-6 col-md-5 col-lg-4">
							<div class="way">
