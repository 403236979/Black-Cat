...
<html>
	<head>
        ...
        <link rel="stylesheet" type="text/css" href="<?php echo $this->ffwbms_vars["wset_ary"]['tpl_path'] ?>css/add.css"/> 
	...
	<div class="row" style="margin-right: 0px;">
		...
		<div class="a"></div>
		<div class="img">
	    	<img src="<?php echo $this->ffwbms_vars["langset_ary"]['image'] ?>"/>
	    </div>
		<div class="container">			
			<div class="title wow">
				<?php $this->ffwbms_get_value(' ffa_sort_db ',' vte1 ', 'id = 21'); ?><!--获得指定分类号的数据-->
        		<?php $this->ffwbms_get_value(' ffa_art_value ',' vte1_art ', 'id = ' .$this->ffwbms_vars["vte1"][sart]); ?><!--获得指定文章号的文章数据    $vte1[sart]：返回分类所属文章号-->
				<p style="font-size: 24px; color: #000000;"><?php echo $this->ffwbms_vars["vte1"][sname] ?></p>
				<p style="    margin-top: 10px;"><?php echo $this->ffwbms_vars["vte1"][keys] ?></p>
				<p><br></p>
				<div style="line-height: 20px;"><?php echo $this->ffwbms_vars["vte1"][atxt] ?></div>
			</div>
			<div class="content">
				
				<div class="content-title wow">
					<?php $this->ffwbms_get_value(' ffa_art_pglist ',' vtemp ', 'pid = 21|ps = 1|ob = 3'); ?><!--获得文章分页列表-->   
					...
					<!---{ffwbms_see $arry }-->
					<p style="font-size: 25px; color: #000000;"><?php echo $this->ffwbms_vars["arry"][stit] ?></p>
				    <p style="margin-top: 15px;"><?php echo $this->ffwbms_vars["arry"][keys] ?></p>
				    <?php } ?>
				</div>
				<div class="content-table">
					<ul class="clearfix">
						<?php $this->ffwbms_get_value(' ffa_art_pglist ',' vtemp ', 'pid = 21|ps = 6|ob = 3'); ?><!--获得文章分页列表-->           
				        <?php foreach( $this->ffwbms_vars["vtemp"][0] as $this->ffwbms_vars["ary"]){ ?>
				        <?php $this->ffwbms_get_value(' ffa_art_value ',' arry ', 'id = ' .$this->ffwbms_vars["ary"][id]); ?>
				        <?php $this->ffwbms_vars["time"] = substr($this->ffwbms_vars["ary"][scdt] , 0 , 10 ) ?>		        				       
						<li class="content-table-td wow col-xs-12 col-sm-6 col-md-4 col-lg-4">
							...
							<div class="content-table-text">
								<p class="content-table-text-p1" >
									<span style="color: rgb(38, 38, 38); font-size: 18px;"><?php echo $this->ffwbms_vars["arry"][atit] ?></span>
								</p>
				                <p class="content-table-text-p2"><?php echo $this->ffwbms_vars["arry"][abs] ?></p>
							</div>
						</li>
				        <?php } ?>
					</ul>								
			...
			<div class="problem">				
					...
					<?php $this->ffwbms_get_value(' ffa_art_value ',' arry ', 'id = ' .$this->ffwbms_vars["ary"][id]); ?>
					<!---{ffwbms_see $arry }-->
					<div class="problem-title wow">
						<?php echo $this->ffwbms_vars["arry"][atxt] ?>
					</div>	
				    <?php } ?>							
				<div class="problem-content wow">					
				    <a target="_blank" href="?tpl=function&pid=15&id=25&sno=4&list=3">功能介绍</a>
				</div>
			</div>
			<div class="main">
				<div class="main-title wow">
					<?php $this->ffwbms_get_value(' ffa_art_pglist ',' vtemp ', 'pid = 21|ps = 1|ob = 2'); ?><!--获得文章分页列表-->   
					<?php foreach( $this->ffwbms_vars["vtemp"][0] as $this->ffwbms_vars["ary"]){ ?>
					<?php $this->ffwbms_get_value(' ffa_art_value ',' arry ', 'id = ' .$this->ffwbms_vars["ary"][id]); ?>
					<!---{ffwbms_see $arry }-->	
					<p style="font-family: '微软雅黑';font-size: 25px; color: #000000;"><?php echo $this->ffwbms_vars["arry"][stit] ?></p>
				    <p><?php echo $this->ffwbms_vars["arry"][keys] ?></p>
				    <?php } ?>					
				</div>
				<div class="main-table wow">
					<ul class="clearfix">						
						<?php $this->ffwbms_get_value(' ffa_art_pglist ',' vtemp ', 'pid = 21|ps = 4|ob = 2'); ?><!--获得文章分页列表-->           
				        <?php foreach( $this->ffwbms_vars["vtemp"][0] as $this->ffwbms_vars["ary"]){ ?>
				        <?php $this->ffwbms_get_value(' ffa_art_value ',' arry ', 'id = ' .$this->ffwbms_vars["ary"][id]); ?>
				        <?php $this->ffwbms_vars["time"] = substr($this->ffwbms_vars["ary"][scdt] , 0 , 10 ) ?>		        				       
						<li class="main-table-td col-xs-12 col-sm-6 col-md-3 col-lg-3">
							<img src="<?php echo $this->ffwbms_vars["wset_ary"]['upload_path'] ?><?php echo $this->ffwbms_vars["arry"][timgary][0][furl] ?>" height="auto" width="100%">
							<p style="text-align: center;"><span style="color: rgb(38, 38, 38); font-size: 16px;"><?php echo $this->ffwbms_vars["arry"][atit] ?></span></p>
							<p style="text-align: center;"><span style="font-size: 12px; color: rgb(191, 191, 191);"><?php echo $this->ffwbms_vars["arry"][abs] ?></span></p>
						</li>
				        <?php } ?>						
					</ul>
				</div>			    
