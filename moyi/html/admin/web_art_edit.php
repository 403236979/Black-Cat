...
<title>在线管理后台 - 网站数据管理 - 网站文章管理 - 编辑</title>
</head>
...
<div id="mainDiv">
	...
	<div id="right"> 
		<div id="current"><span style="float:right;"><a href="web_art_pglist.php?pid=<?php echo $ol_set['get_ary']['pid']; ?>&pg=<?php echo $ol_set['get_ary']['pg']; ?>" class="addStyle">返回文章列表</a></span>&nbsp;&nbsp;&nbsp;&nbsp;网站文章管理 - 编辑</div>
		<div id="form">
<?php
	if(!empty($dblang)) {
		$temp=ol_getwebartdb($dblang,$ol_set['get_ary']['id']);
		$temp['indt']=substr($temp['indt'],0,10);
		//echo '<pre>'; print_r($temp); echo '</pre><br />';
	}
?>

<form action="web_art_edit.php" method="post">

	<input name="act" type="hidden" value="web_art_edit" />
	<fieldset>
    <div class="ceng">
		...
		<input id="id" name="info[id]" type="hidden" value="<?php echo $ol_set['get_ary']['id']; ?>" />

        <label for="atit" class="label1">标题:</label>
		<input name="info[atit]" type="text" id="atit" size="60" value="<?php echo $temp['atit']; ?>" style="width:85%" /><br />

		<label for="stit" class="label1">短标题:</label>
		<input name="info[stit]" type="text" id="stit" size="30" style="min-width:200px;width:85%"  value="<?php echo $temp['stit']; ?>" />&nbsp;&nbsp;&nbsp;&nbsp;
		<label for="stit" class="label1">短标题:</label>

		<label for="indt" class="label1" >有效时间:</label>
		<input name="info[indt]" type="text" id="indt" style="width:119px" onfocus="ffcms_setDate(this,<?php echo $ol_set['wset_ary']["currY"]; ?>,2019)" value="<?php echo $temp['indt']; ?>" readonly><br />
		...
		<input name="info[auth]" type="text" id="auth" size="60" value="<?php echo $temp['auth']; ?>"  style="width:85%;" /><br />
		<label for="sour" class="label1">来源:</label>
		<input name="info[sour]" type="text" id="sour" size="60" value="<?php echo $temp['sour']; ?>" style="width:85%;"  /><br />
		<label for="keys" class="label1">关键字:</label>
		<input name="info[keys]" type="text" id="keys" size="60" value="<?php echo $temp['keys']; ?>"  style="width:85%;" /><br />
		<label for="abs" class="label1">描述:</label>
		<input name="info[abs]" type="text" id="abs" size="60" value="<?php echo $temp['abs']; ?>"  style="width:85%;" /><br />
		<input type="checkbox" value="1" id="sta1_1" name="info[sta1_1]"<?php if($temp['sta1_1']==1) echo ' checked="checked"'; ?> /> 是否置顶&nbsp;&nbsp;
		<input type="checkbox" value="1" id="sta1_2" name="info[sta1_2]"<?php if($temp['sta1_2']==1) echo ' checked="checked"'; ?> /> 是否推荐&nbsp;&nbsp;
		<input type="checkbox" value="1" id="sta2_1" name="info[sta2_1]"<?php if($temp['sta2_1']==1) echo ' checked="checked"'; ?> /> 是否可回复&nbsp;&nbsp;
        <input id="sta2_2" name="info[sta2_2]" type="hidden" value="0" />
		<input type="checkbox" value="1" id="sta4_1" name="info[sta4_1]"<?php if($temp['sta4_1']==1) echo ' checked="checked"'; ?> /> 标题加粗&nbsp;&nbsp;
		<input type="checkbox" value="1" id="sta4_2" name="info[sta4_2]"<?php if($temp['sta4_2']==1) echo ' checked="checked"'; ?> /> 标题加红&nbsp;&nbsp;
		<input type="checkbox" value="2" id="ifyn" name="info[ifyn]"<?php if($temp['ifyn']==2) echo ' checked="checked"'; ?> /> 是否发布&nbsp;&nbsp;<br/>
		<label for="abs" class="label1">正文:</label>
        <textarea id="atxt" name="info[atxt]" width="100%"><?php echo $temp['atxt']; ?></textarea>
