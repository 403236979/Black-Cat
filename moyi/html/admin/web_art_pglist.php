...
<title>在线管理后台 - 网站数据管理 - 网站文章分页列表</title>
</head>
...
<div id="mainDiv">
	...
	<div id="right"> 
		<div id="current"><span style="float:right;"><a href="web_art_add.php?pid=<?php echo $ol_set['get_ary']['pid']; ?>&pg=<?php echo $ol_set['get_ary']['pg']; ?>" class="addStyle">增加新资料</a></span>&nbsp;&nbsp;&nbsp;&nbsp;网站文章分页列表:</div>
		<div id="form">

<table id="list" width="100%" border="1">
<tr><td width="8%" style="background:#919fb9;color:#fff;">序号</td><td width="40%" style="background:#919fb9;color:#fff;text-align:left;padding-left:10px;">栏目名称</td><td width="14%"  style="background:#919fb9;color:#fff;">最后修改时间</td><td width="8%"  style="background:#919fb9;color:#fff;">查看</td><td width="14%" style="background:#919fb9;color:#fff;">有效时间</td><td width="8%"  style="background:#919fb9;color:#fff;">状态</td><td width="8%" style="background:#919fb9;color:#fff;">删除</td> </tr>
<?php
	if(!empty($dblang)) {
	...
	$i_tmp=1;
	 foreach($temp[0] as $ary){
		...
		echo '<td class="leftStyle"><a href="web_art_edit.php?id='.$ary['id'].'&pid='.$ol_set['get_ary']['pid'].'&pg='.$ol_set['get_ary']['pg'].'">'.$ary['atit'].'</a></td>';
		echo '<td style="width:160px;" style="text-align:center;">'.$ary['amdt'].'</td>';
		echo '<td style="width:60px;" style="text-align:center;">'.$ary['acou'].'</td>';
		echo '<td style="width:160px;" style="text-align:center;">'.$ary['indt'].'</td>';
		switch($ary['ifyn']){
			case 1:
		...
		echo '<td class="icon"><a title="删除" href="web_art_del.php?refer=web_art_pglist_del&id='.$ary['id'].'&pid='.$ary['pid'].'&pg='.$ol_set['get_ary']['pg'].'" onclick="delcfm()"><span style="background-position:0 -26px;"> </span></a></td>';
		echo '</tr>';
		$i_tmp++;
	}
	//echo '<pre>'; print_r($temp); echo '</pre><br />';
