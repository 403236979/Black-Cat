...
<![endif]–>
<title>在线管理后台 - 留言管理 - 留言分类管理 - 列表</title>
</head>
...
//echo '<pre>'; print_r($temp); echo '</pre><br />';
	...
	$i_tmp=1;
	 foreach($temp as $ary){
		...
		if($i_tmp<10) echo '0';
		echo $i_tmp.'</td>';
		echo '<td>'.$ary['id'].'</td>  <td class="leftStyle"><a href="mess_sort_edit.php?id='.$ary['id'].'">'.$ary['sname'].'</a></td>';
		...
		echo '<td class="icon"><a href="mess_sort_edit.php?id='.$ary['id'].'"><span style="background-position:0 -117px;"> </span></a></td>';
		echo '<td class="icon"><a href="mess_sort_del.php?refer=mess_sort_del&id='.$ary['id'].'" onclick="delcfm()"><span style="background-position:0 -26px;"> </span></a></td>';
		echo '</tr>';
		$i_tmp++;
	}
	//echo '<pre>'; print_r($temp); echo '</pre><br />';
