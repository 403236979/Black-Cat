...
if(!empty($ol_set['post_ary'])){
	$temp=ol_bbs_sort_edit($dbffe4,$ol_set['post_ary']);
...
<![endif]–>
<title>在线管理后台 - 论坛管理 - 版块管理 - 编辑</title>
</head>
...
<div id="mainDiv">
	...
	<div id="right"> 
		<div id="current"><span style="float:right;"><a href="bbs_sort_list.php" class="addStyle">返回版块列表</a></span>&nbsp;&nbsp;&nbsp;&nbsp;版块管理 - 编辑版块</div>
		<div id="form">
<?php
$temp=ol_bbs_sort_getvalue($dbffe4,$ol_set['get_ary']['id']);
//echo '<pre>'; print_r($temp); echo '</pre><br />';
?>
<form action="bbs_sort_edit.php?id=<?php echo $ol_set['get_ary']['id']; ?>" method="post">
	<fieldset>
    <div class="ceng">
		<legend>编辑版块</legend>
        <input name="info[id]" type="hidden" value="<?php echo $temp['id']; ?>" />
        <input name="info[pid]" type="hidden" value="<?php echo $temp['pid']; ?>" />
        <label for="sname" class="label1">版块名:</label>
		<input name="info[sname]" type="text" id="sname" size="30" value="<?php echo $temp['sname']; ?>" /><br />
		<label for="sname" class="label1">版块说明:</label>
		<input name="info[snote]" type="text" id="snote" style="width:465px;" size="60" value="<?php echo $temp['snote']; ?>" /><br />
