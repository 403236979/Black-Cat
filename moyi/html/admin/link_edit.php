...
<![endif]–>
<title>在线管理后台 - 友情链接管理 - 友情链接管理 - 友情链接编辑</title>
</head>
...
<div id="mainDiv">
	<div id="centerDiv">
<?php
	$temp=ol_getlinkdb($dbffe2,'link',$ol_set['get_ary']['id']);
	$ol_set['get_ary']['pid']=$temp['pid'];
?>
	<div id="right"> 
		<div id="current"><span style="float:right;"><a href="link_list.php<?php if($ol_set['get_ary']['pid']==0 || $ol_set['get_ary']['pid']=='') echo '?pid=1'; else echo '?pid='.$ol_set['get_ary']['pid']; ?>" class="addStyle">返回友情链接列表</a></span>&nbsp;&nbsp;&nbsp;&nbsp;友情链接管理 - 编辑</div>
		<div id="form">

<form action="link_edit.php" method="post">

	<input name="act" type="hidden" value="link_edit" />
	<fieldset>
    <div class="ceng">
		<h3>友情链接</h3>
...
?>
		...
		<input name="info[ltit]" type="text" id="ltit" size="40" value="<?php echo $temp['ltit']; ?>" /><br />
		<label for="lurl" class="label2">友情链接转向地址:</label>
		<input name="info[lurl]" type="text" id="lurl" size="60" value="<?php echo $temp['lurl']; ?>" /><br />
		<label for="llogo" class="label2">友情链接图片地址:</label>
		<input name="info[llogo]" type="text" id="llogo" size="60" value="<?php echo $temp['llogo']; ?>" /><br />
		<label for="lhint" class="label2">鼠标指向提示字符:</label>
		<input name="info[lhint]" type="text" id="lhint" size="60" value="<?php echo $temp['lhint']; ?>" /><br />
		<label for="lstate" class="label2">友情链接启用时间:</label>
		<input name="info[lstate]" type="text" id="lstate" style="width:80px" onfocus="ffcms_setDate(this,<?php echo $ol_set['wset_ary']["currY"]; ?>,2019)" value="<?php if (isset($temp['lstate'])) echo $temp['lstate']; else echo "2019-12-31"; ?>" readonly>
		<label for="lstop">友情链接结束时间:</label>
		<input name="info[lstop]" type="text" id="lstop" style="width:80px" onfocus="ffcms_setDate(this,<?php echo $ol_set['wset_ary']["currY"]; ?>,2019)" value="<?php if (isset($temp['lstop'])) echo $temp['lstop']; else echo "2019-12-31"; ?>" readonly><br />
