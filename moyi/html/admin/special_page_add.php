...
<title>在线管理后台- 网站特殊管理 - 特殊页面管理 - 增加</title>
<style type="text/css"> 
...
<div id="mainDiv">
	...
	<div id="right"> 
		<div id="current"><span style="float:right;"><a href="special_page_list.php" class="addStyle">返回特殊页面列表</a></span>&nbsp;&nbsp;&nbsp;&nbsp;特殊页面管理 - 增加</div>
		<div id="form">

<form action="special_page_add.php" method="post">

	<input name="act" type="hidden" value="special_page_add" />
   
	<fieldset> 
    <div class="ceng">
		<h3>增加新特殊页面</h3>
		<label for="stit" class="label1">标识名:</label>
		<input name="info[stit]" type="text" id="stit" size="30" value="" onblur="yzstit(this.value)" style="width:388px;" /><br />
        <label for="atit" class="label1">标题:</label>
		<input name="info[atit]" type="text" id="atit" size="60" value="" /><br />
        ...
        <textarea id="atxt" name="info[atxt]"></textarea><br>
       <center class="center">
...
function yz(obj){
	...
	$.ajax({
		type: "GET",
		url: "ajaxs.php?act=specialpagestit&stit="+$('#stit').val(),
		cache: false,
		success: function(msg){
			if(msg!='ok') {alert(msg);return false;};
			if($.trim($('#stit').val())==''){alert('标识名不能为空，请输入标识名！'); $("#stit").focus().select(); return false;};
...
function yzstit(stit){
	$.ajax({
		type: "GET",
		url: "ajaxs.php?act=specialpagestit&stit="+stit,
		cache: false,
		success: function(msg){
			if(msg!='ok') alert(msg);
		}
