...
<![endif]–>
<title>在线管理后台 - 上传文件图片管理 - 编辑</title>
</head>
...
<div id="mainDiv">
	...
	<div id="right"> 
		<div id="current"><span style="float:right;"><a href="web_img_pglist.php?pg=<?php echo $ol_set['get_ary']['pg']; ?>" class="addStyle">返回上传文件图片管理</a></span>&nbsp;&nbsp;&nbsp;&nbsp;编辑文件或图片</div>
		<div id="form">
...
<form action="web_img_edit.php" method="post" >

	<input name="act" type="hidden" value="web_img_edit" />
	<fieldset>
    <div class="ceng">
        <?php /* furl,fext */ ?>
        <input id="pg" name="pg" type="hidden" value="<?php echo $ol_set['get_ary']['pg']; ?>" />
        <input id="id" name="info[id]" type="hidden" value="<?php echo $temp['id']; ?>" />
        <label for="ftit" class="label1">标题:</label>
		<input name="info[ftit]" type="text" id="ftit" size="60" value="<?php echo $temp['ftit']; ?>" style="width:211px;" /><br />
        <label for="ftxt" class="label1">说明:</label>
        <textarea id="ftxt" name="info[ftxt]" style="width:221px; height:100px;"><?php echo $temp['ftxt']; ?></textarea><br />
        <label for="fnote" class="label1">注释:</label>
        <textarea id="fnote" name="info[fnote]" style="width:221px; height:100px;"><?php echo $temp['fnote']; ?></textarea><br />
		<label for="fpwd" class="label1">密码:</label>
		<input name="info[fpwd]" type="text" id="fpwd" size="30" value="<?php echo $temp['fpwd']; ?>" /><br />
        <center class="center">
