...
function ffa_art_toplist($db,$parary){
	if($parary['pid']==""||$parary['pid']==0) return ; else {
		...
		$colums='id,pid,atit,stit,auth,sour,timg,scdt,keys,abs,acou,sta1,sta2,sta3,sta4';
		if($parary['zd']==1){
...
function ffa_art_astoplist($db,$parary){
	...
	if($parary['pid']==""||$parary['pid']==0) return ; else {
		...
		//$rece="(pid=".$parary['pid'].")";
		$dbc=$db['dblang'];
		$pids=explode(' ',$parary['pid']);
		$rece="(pid=".$pids[0].")";
		$ff_i=0;
		foreach($pids as $item_pid){
			if(!empty($item_pid)){
				if($ff_i){
					$rece.=" or (pid=".$item_pid.")";
				}
				$ff_i++;
				$rece.=callbackergodic2($dbc,"sort",$item_pid,"id");
			}
		}
...
指定文章号的回复
id：必选项，指定文章号，为空或为0时，返回空
t：可选项，多长时间内同一IP不许重复回复，值为数字，单位为分钟，为0时无间隔时间，为空时默认间隔时间=15分钟
		$t = 可选，
注意：	表单提交的内容放入info组中，
	例：<input name="info[aaa]" type="text" value="aaaaaa" />
	atit不能为空，值应该为文章标题
	post['info']['atit']=回复文章的标题
	post['info']['auth']=自定义
	post['info']['sour']=自定义
	post['info']['keys']=自定义
	post['info']['atxt']=回复正文

必须有验证码，验证码会话号名：VerifyCode
表单验证码输入框名：VerifyCode
返回：
-16;//未登录用户
-15;//回复所属文章号不能为空
-14;//表单数据不能为空
...
******************************************/
function ffa_art_repl($db,$parary){
	//id,uid,atyp=1,raid,atit,stit=ip,auth,sour,keys,atxt,scdt,ifyn

	if(empty($parary['cook_ary']['ul']['s0'])) return -16; else //回复用户号号

	if(trim($parary['id'])==""||$parary['id']==0) return -15; else //回复所属文章号

	if(empty($parary['post_ary'])) return -14; else //表单数据

	if(strtolower(trim($_SESSION['VerifyCode']))!=strtolower(trim($parary['post_ary']['VerifyCode']))){
			return -12;//验证码错误
	}else{
		if(empty($parary['post_ary']['info']['atit'])){
			$temp=-11;//标题不能为空
		}else{
			$isok=true;
			$DefaultTimeInterval=15; //默认间隔时间，分钟
			if(empty($parary['t'])){
				$parary['t']=$DefaultTimeInterval;
			}else{
				if(!is_numeric($parary['t']))$parary['t']=$DefaultTimeInterval;
				if($parary['t']<0)$parary['t']=$DefaultTimeInterval;
				if($parary['t']==0){//
					$isok=true;
				}else{
					$temp ="((raid=".trim($parary['id']).") AND (atyp=1)";
					$temp.=" AND (stit='".get_user_IP()."')";
					$temp.=" AND (scdt>='".date('Y-m-d H:i:s',strtotime("-".$parary['t']." minute"))."'))";
					$res=$db['dblang']->getlist('art',$temp,'id','1','ORDER BY id DESC');
					if(!empty($res)) $isok=false;
				}
			}

			if($isok==false){
				$temp=-13;//同一IP指定时间内只能留言一次
			}else{
				unset($updatedb); //清空数组
	//uid,atyp=1,raid,atit,stit=ip,auth,sour,keys,atxt,ifyn
				...
				if(!isDigit($parary['post_ary']['info']['pid'])||$parary['post_ary']['info']['pid']==0)
					$parary['post_ary']['info']['pid']=1;
				if(!isDigit($parary['post_ary']['info']['raid']))$parary['post_ary']['info']['raid']=0;

				$updatedb['uid']=ffStrDE($parary['cook_ary']['ul']['s0'],'');
				$updatedb['atyp']=1;
				$updatedb['raid']=$parary['id'];
				$updatedb['atit']='RE: '.$parary['post_ary']['info']['atit'];
				if($parary['post_ary']['info']['auth']!='')
					$updatedb['auth']=$parary['post_ary']['info']['auth'];
				if($parary['post_ary']['info']['sour']!='')
					$updatedb['sour']=$parary['post_ary']['info']['sour'];
				if($parary['post_ary']['info']['keys']!='')
					$updatedb['keys']=$parary['post_ary']['info']['keys'];
				if($parary['post_ary']['info']['atxt']!='')
					$updatedb['atxt']=$parary['post_ary']['info']['atxt'];
				$updatedb['scdt']=date('Y-m-d H:i:s',time());
				$updatedb['stit']=get_user_IP();//回复IP
				$updatedb['ifyn']=2;//是否可用
				//echo '<pre>'; print_r($updatedb); echo '</pre>'; exit;
				$temp=$db['dblang']->execinsert('art',$updatedb);
				//$temp=$updatedb;
			}
		}
		return $temp;
...
id：必选项，指定文章号，为空或为0时，返回空
top：可选，前X条记录，为空或为0时，列全部记录
ob：可选，排序设置，z=正序，其它=倒序，默认倒序
******************************************/
function ffa_art_relist($db,$parary){
echo $parary['id'].'<br />';
echo $parary['top'].'<br />';
	if(trim($parary['id'])==""||$parary['id']==0) return ; else {
		$colums='id,uid,atit,stit,auth,sour,keys,atxt,scdt,atit';
		if(trim($parary['top'])==""||$parary['top']==0) $limits='';
			else $limits=trim($parary['top']);
		if(strtolower($parary['ob'])=="z") $obstr="ORDER BY scdt ";
			else $obstr="ORDER BY scdt DESC";
		$temp=$db['dblang']->getlist('art','(raid='.trim($parary['id']).') AND (atyp=1)',$colums,$limits,$obstr);
		return $temp;
	}/**/
}
...
function ffa_art_repglist($db,$parary){
	if(trim($parary['id'])==""||$parary['id']==0) return ; else {
		...
		$temp=$db['dblang']->getpglist('art','(raid='.trim($parary['id']).') AND (atyp=1)',$colums,$obstr,$parary['pg'],$parary['ps'],$parary['os']);
		return $temp;
	}/**/
}
...
**************************************************************/
//echo '<script type="text/javascript" src="./admin/images/jquery-1.7.2.min.js"></script>';
//echo '<script>$(function(){$("body>a:first-of-type").remove()})</script>';
